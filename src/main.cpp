...
camera camera(glm::vec3(0.0f, 10.0f, -5.0f), glm::vec3(0.0f, 1.0f, 0.0f), 90.0f,
              -20.0f);
...
int main() {
  ...
  while (!glfwWindowShouldClose(window.get())) {
    ...
    glm::mat4 projection = glm::perspective(
        glm::radians(45.0f), (float)global::scr_width / (float)global::scr_height, 0.1f, 100.0f);

    glUniformMatrix4fv(glGetUniformLocation(shader.get(), "model"), 1, GL_FALSE,
                       glm::value_ptr(model));
    glUniformMatrix4fv(glGetUniformLocation(shader.get(), "view"), 1, GL_FALSE,
                       glm::value_ptr(view));
    glUniformMatrix4fv(glGetUniformLocation(shader.get(), "projection"), 1,
